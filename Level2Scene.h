...
#include <iostream>
#include "Scene.h" // Cada nivel hereda de Scene
...
//      std::cout << "Level 2 Started!" << std::endl;
    ...
    {
      if (_bgSprite.getPosition().y == 800)
        levelComplete=true;

      if (levelComplete)
      {
          // Llamar al método de SceneManager para cambiar de escena
//          _sceneManager->SceneManager::changeScene(new Level3Scene());  // O la escena siguiente
      }

      if (Gameplay::getInstance()->getVidas() < 0) {
          // Llamar al método de SceneManager para cambiar de escena
//          _sceneManager->SceneManager::changeScene(new Level2Scene());  // O la escena siguiente
      }
...
//    SceneManager* _sceneManager; // Usamos SceneManager para cambiar de escena
